...
\begin{statementbox}
On dit que $A=(a_{ij})_{1\leq i,j\leq n}\in\M_n(\K)$, une matrice est en damier ssi
\[
\forall(i,j)\in\intervalint{1}{n}^2,\ i+j\text{ est impair alors }a_{ij}=0.
\]
Je note $\DM_n(\K)$ l'ensemble de ces matrices.
\begin{enumerate}[label=\alph*.]
  \item Mq $\DM_n(\K)$ est un ss-ev de $\M_n(\K)$ stable par produit.
...
On a $a_mA^m+\cdots+a_nA^n=0$. ième multiplication par $(A^{-1})^m$ :
\[
...
considérer le polynôme annulateur non nul de $M$). Alors
...
\[
S_\theta(e_2)=(-\cos\theta)e_1-\frac{u_\theta}{\sin\theta}
=-2\cot(\theta)e_1-e_2.
\]
Ainsi
\[
...
\begin{enumerate}[label=\arabic*)]
  ...
  \[
  \tr A=0\quad\text{ssi}\quad\exists(B,C)\in\M_n(\C)^2,quad A=BC-CB.
  \]
\end{enumerate}
\end{statementbox}
...
\[
\exists P\in\GL_n(\C),qquad A=P\underbrace{A'}_{\text{diag. nulle}}P^{-1}.
\]
...
\begin{enumerate}[label=\alph*)]
  \item Exprimer, pour $n\in\N$, $n!$ en fct de $(d_0,\ldots,d_n)$.
\end{enumerate}
\end{statementbox}
...
\]
est la probabilité pour qu'une permutation de $\mathfrak S_n$ (muni de la probabilité
...
\end{pmatrix}
\]
possédant $n^2-n+1$ coefficients impairs. En effectuant
